...
function! s:run(exe) abort "{{{
  call jobstart([a:exe] + get(g:, 'uracil_cli_args', []), { 'rpc': v:true, 'cwd': s:repo, })
endfunction "}}}
...
function! s:built(code) abort "{{{
  if a:code == 0
    ...
    call s:error('Failed to build uracil')
  endif
endfunction "}}}
...
function! s:fetched(code) abort "{{{
  if a:code == 0
    ...
    call s:error('Failed to fetch the uracil executable from github')
  endif
endfunction "}}}
...
elseif filereadable(s:gh_exe)
  ...
  if executable('nix') && !get(g:, 'uracil_fetch_bin', 0)
